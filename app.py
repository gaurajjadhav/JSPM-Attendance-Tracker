...
def seed_if_empty():
	...
	for name, phone in phone_map.items():
		c.execute('UPDATE teachers SET phone = ? WHERE name = ?', (phone, name))
	db.commit()
...
def login():
	...
	if request.method == 'POST':
		...
		if role == 'student':
			...
			row = db.execute('SELECT * FROM teachers WHERE phone = ?', (username,)).fetchone()
...
def student_dashboard():
	...
	# subject-wise percentages in period - show all defined subjects
	all_subjects = [
		{'subject': 'FBDA', 'code': '230GDIM22'},
		{'subject': 'BSE', 'code': '230USYB01'},
		{'subject': 'Wireless Communication', 'code': '230GETB38'},
		{'subject': 'E-Commerce', 'code': '230VBCB14'},
		{'subject': 'CCF', 'code': '250GCAM65'}
	]
	percents = []
	for sub_info in all_subjects:
